# deprecated but i'll keep it around for a little while
{ config, lib, pkgs, ... }: {
  options.basashi.terminal.zsh.enable = lib.mkEnableOption "enable zsh as the main shell";
  config = lib.mkIf config.basashi.terminal.zsh.enable {
    environment = {
